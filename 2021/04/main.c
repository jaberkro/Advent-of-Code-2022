...
void	ft_read_line(void)
{
	...
	close(stream);
	while (j < 100) // amount of inputs you want to check
	{
		stream = fopen("input2.txt", "r");
		getline(&line, &len, stream);
		getline(&line, &len, stream);
		...
		while (i < total)
		{
			...
			while (o < 5)
			{
				...
				{
					printf("bingo found! horizontal. card: %d at line %d\n", i + 1, 2 + (i + 1) * 6 - 5);
					printf("Last number that was called: %d", atoi(inputs[m - 1]));
					return;
				}
				o++;
			}
			while (p < 5)
			{
				...
				{
					printf("bingo found! vertical. card: %d at line %d\n", i + 1, 2 + ((i + 1) * 6 - 5));
					printf("Last number that was called: %d", atoi(inputs[m - 1]));
					return;
				}
				p++;
