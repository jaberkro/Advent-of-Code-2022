...
/*                                                   +#+                      */
/*   Created: 2022/12/05 20:29:02 by jaberkro      #+#    #+#                 */
/*   Updated: 2022/12/05 20:54:50 by jaberkro      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	move_stacks(std::string line, std::string *stacks)
{
	...
	amount = (unsigned long)stoi(line.substr(5, line.find(" from ")));
	from = stoi(line.substr(line.find(" from ") + 6, line.find(" to "))) - 1;
	to = stoi(line.substr(line.find(" to ") + 4, line.length())) - 1;
	for (unsigned long i = 0; i < amount; i++)
	{
		if (stacks[from].length() - amount + i >= 0)
			stacks[to] += stacks[from].at(stacks[from].length() - amount + i);
	}
	for (unsigned long i = 0; i < amount; i++)
